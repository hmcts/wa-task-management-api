...
*   Search & Indexing: The project retains the legacy signature-based `search_index` GIN path behind a feature flag while the replacement path queries `tasks` and `task_roles` directly with B-tree indexes. For full technical details on queries, relations, and indexes, refer to [docs/database_search_context.md](docs/database_search_context.md).
