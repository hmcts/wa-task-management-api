...
To run and publish the consumer Pact tests locally in one step, use the combined
task with `clean`. Cleaning removes stale files so `build/pacts` contains only
pacts generated by the current run:

```
./gradlew clean runAndPublishConsumerPactTests
```

Note: The broker URL defaults to `http://localhost:80`, and the branch name and tag
default to `Dev`.
