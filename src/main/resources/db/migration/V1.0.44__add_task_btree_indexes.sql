-- Counts probe the roles of matching tasks instead of scanning historical roles.
-- Keep role_name as payload so a role-name IN list does not multiply index searches.
CREATE INDEX CONCURRENTLY IF NOT EXISTS task_roles_search_manage_task_lookup_idx
    ON cft_task_db.task_roles (task_id)
    INCLUDE (role_name)
    WHERE manage;

CREATE INDEX CONCURRENTLY IF NOT EXISTS task_roles_search_available_task_lookup_idx
    ON cft_task_db.task_roles (task_id)
    INCLUDE (role_name, authorizations)
    WHERE own AND claim;

CREATE INDEX CONCURRENTLY IF NOT EXISTS search_available_tasks_count_idx
  ON cft_task_db.tasks USING btree (jurisdiction, work_type)
  INCLUDE (task_id, security_classification, region, location, case_id, task_type, role_category)
  WHERE state = 'UNASSIGNED' AND indexed AND assignee IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS search_task_filters_idx
  ON cft_task_db.tasks USING btree (
                                    state,
                                    jurisdiction,
                                    work_type,
                                    role_category,
                                    location,
                                    region,
                                    major_priority,
                                    priority_date,
                                    minor_priority,
                                    task_id
    )
  WHERE state IN ('ASSIGNED', 'UNASSIGNED') AND indexed;

CREATE INDEX CONCURRENTLY IF NOT EXISTS search_active_tasks_sort_idx
  ON cft_task_db.tasks USING btree (major_priority, priority_date, minor_priority, task_id)
  WHERE state IN ('ASSIGNED', 'UNASSIGNED') AND indexed;

CREATE INDEX CONCURRENTLY IF NOT EXISTS search_available_tasks_sort_idx
  ON cft_task_db.tasks USING btree (
                                    major_priority,
                                    priority_date,
                                    minor_priority,
                                    task_id
    )
  INCLUDE (
    jurisdiction,
    work_type,
    role_category,
    location,
    region,
    case_id,
    task_type,
    security_classification
    )
  WHERE state IN ('ASSIGNED', 'UNASSIGNED')
    AND indexed
    AND assignee IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS search_assignee_idx
  ON cft_task_db.tasks USING btree (assignee, major_priority, priority_date, minor_priority, task_id)
  WHERE state IN ('ASSIGNED', 'UNASSIGNED') AND indexed AND assignee IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS search_task_type_idx
  ON cft_task_db.tasks USING btree (task_type, major_priority, priority_date, minor_priority, task_id)
  WHERE state IN ('ASSIGNED', 'UNASSIGNED') AND indexed;
