...
begin
  ...
  if l_result_action = 'update' then
    ...
    set   task_name = l_task.task_name,
          ...
          wait_time_days = case when (l_task.state='ASSIGNED') then (l_task.last_updated_timestamp::date - reportable_task.created_date) else reportable_task.wait_time_days end,
